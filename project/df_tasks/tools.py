...
def preload_datasets(paths: List[str]) -> str:
    """Loads CSV files into a global cache if not already loaded.
      Helps to efficiently manage datasets by loading them once
      and storing them in memory for future use. Prevents token waste.
...
def get_dataset_summaries(dataset_paths: List[str]) -> List[Dict[str, Any]]:
    """Analyze multiple CSV files and return metadata summaries for each.
      ...
      Returns:
        List[Dict[str, Any]]
          A list of dictionaries, one per dataset, each containing:
...
def call_dataframe_method(file_name: str, method: str) -> str:
   ...
   Makes is possible to run simple DataFrame methods like 'head', 'tail', or 'describe'
   on a dataset that has already been loaded and cached using the tool 'preload_datasets'.
